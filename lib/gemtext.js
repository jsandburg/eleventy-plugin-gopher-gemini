...
export function toGemtext(markdown = "") {
  ...
  for (const line of lines) {
    ...
    if (/^>\s?/.test(line)) {
      out.push(`> ${stripInlineLinks(line.replace(/^>\s?/, ""), pendingLinks)}`);
      continue;
    }

    const listItem = line.match(/^\s*(?:[-*]|\d+\.)\s+(.*)$/);
    if (listItem) {
      out.push(`* ${stripInlineLinks(listItem[1], pendingLinks)}`);
      continue;
    }
    ...
    out.push(stripInlineLinks(line, pendingLinks));
  }
  flushLinks();
...
function stripInlineLinks(text, collector) {
