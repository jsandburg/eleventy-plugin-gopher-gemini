...
export function hasOutput(itemOrData, protocol) {
  ...
  // `outputs: gopher` (a bare YAML string) means the same as `[gopher]` --
  // failing open here would ship a page to protocols the author excluded.
  if (typeof outputs === "string") return outputs === protocol;
