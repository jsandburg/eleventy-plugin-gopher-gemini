...
import { processInline } from "./inline.js";
...
export function toGopherText(markdown = "", { maxLineWidth = 70 } = {}) {
  ...
  for (const line of lines) {
    ...
    if (heading) {
      ...
      out.push("");
      out.push(text);
      out.push("-".repeat(Math.max(0, Math.min(text.length, maxLineWidth))));
      continue;
    }
...
function wrap(text, width) {
  ...
  for (const word of text.split(/\s+/).filter(Boolean)) {
    if (width > 0 && word.length > width) {
      // Hard-break a word longer than the width (usually a bare URL) so no
      // output line ever exceeds maxLineWidth.
...
function collapse(lines) {
  // A line consisting of just "." is the RFC 1436 type-0 end-of-file marker;
  // pad it with a trailing space so a strict server/client can't mistake it
  // for the terminator mid-document.
  const text = lines
    .map((line) => (line === "." ? ". " : line))
    .join("\n")
    .replace(/\n{3,}/g, "\n\n")
